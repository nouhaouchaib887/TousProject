...
import { fieldIntervention } from '@/types'
import { getStaff, getRoles } from '../../../../Staff/api/staffService'




interface fieldInterventionsSectionProps {
  value: fieldIntervention [];
    onChange: (value: fieldIntervention []) => void;
}

export default function FieldInterventionsSection({ value,onChange }: fieldInterventionsSectionProps) {
...
const [availableRoles, setAvailableRoles] = useState<any[]>([])
  ...
  return (
            ...
            <Card className="p-6 border border-slate-200">
              <h3 className="text-lg font-bold mb-6 text-slate-900 uppercase tracking-wide text-sm">Interventions Terrain
              </h3>
             ...
             {/* List of Interventions */}
                        ...
                        <div key={intervention.id} className="p-4 bg-slate-50 rounded-xl border border-slate-100 space-y-4">
                          <div className="flex items-center justify-between">
                            <div className="flex-1">
                              <p className="text-sm font-bold text-slate-900">Intervention {idx + 1} - {intervention.intervention_date || 'Sans date'}</p>
                              <p className="text-xs text-slate-500 italic">
                                 {intervention.staff_with_roles?.map((ar: any) => ar.agent?.full_name).join(", ") || "Aucun agent"}
                              </p>
                              {intervention.observations && !isEditing && (
                                <p className="text-xs text-slate-400 mt-1 line-clamp-1 italic">Obs: {intervention.observations}</p>
                              )}
                            </div>
                          ...
                          {isEditing && (
                            <div className="pt-4 border-t border-slate-200/60 space-y-4 bg-white/50 p-3 rounded-lg">
                              ...
                              <div className="space-y-1">
                                ...
                                <Input
                                  type="date"
                                value={toDateInputValue(intervention.intervention_date)}
                                  onChange={(e) => {
                                    const updated = value.map((i: any) => 
                                      i.id === intervention.id ? { ...i, intervention_date: e.target.value } : i
                                    );
                                    onChange(updated);
                                  }}
                              ...
                              {/* 2. Observations */}
                              <div className="space-y-1">
                                <Label className="text-[10px] font-bold text-slate-400 uppercase">Observations</Label>
                                <textarea
                                  placeholder="Observations..."
                                  value={intervention.observations || ''}
                                  onChange={(e) => {
                                    const updated = value.map((i: any) => 
                                      i.id === intervention.id ? { ...i, observations: e.target.value } : i
                                    );
                                    onChange(updated);
                                  }}
              ...
              <div className="space-y-6 p-6 border border-dashed border-slate-200 rounded-2xl bg-slate-50/30">
                <div className="space-y-4">
                  ...
                  {newIntervention.staff_with_roles && newIntervention.staff_with_roles.length > 0 && (
                    <div className="space-y-2">
                      {newIntervention.staff_with_roles.map((item: any, idx: number) => (
                      ...
                      <p className="text-xs text-slate-500 uppercase font-medium">Rôle: {item.role.name}</p>
                          ...
                          <Button
                            ...
                            onClick={() => {
                              setNewIntervention({
                                ...newIntervention,
                                staff_with_roles: newIntervention.staff_with_roles.filter((_: any, i: number) => i !== idx),
                              })
                            }}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                      ...
                      <Select 
                         value={newIntervention.selectedAgent?.id || ''}
                          onValueChange={(agent_id) => {
                          const agent = availableAgents.find((a) => a.id === agent_id)
                          if (!agent) return
                      ...
                      >
                        <SelectTrigger className="h-10 bg-white">
                          <SelectValue placeholder="Choisir..." >
                             {newIntervention.selectedAgent?.full_name}
                          </SelectValue>
                        </SelectTrigger>
                        <SelectContent>
                          {availableAgents.map((agent) => (
                    ...
                    <div className="space-y-1">
                     ...
                     <Select 
                         value={newIntervention.selectedAgentRole?.id || ''}
                          onValueChange={(role_id) => {
                      ...
                      >
                        <SelectTrigger className="h-10 bg-white">
                          <SelectValue placeholder="Choisir..." >
                             {newIntervention.selectedAgentRole?.name}
                         </SelectValue>
                        </SelectTrigger>
                        <SelectContent>
                          {availableRoles.map((role) => (
                  ...
                  <Button
                    ...
                    variant="outline"
                    onClick={() => {
                      if (newIntervention.selectedAgent && newIntervention.selectedAgentRole) {
                        const updatedStaffWithRoles = [...(newIntervention.staff_with_roles || [])]
                        updatedStaffWithRoles.push({
                     
                       agent: newIntervention.selectedAgent,
                       role: newIntervention.selectedAgentRole,
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setNewIntervention({ ...newIntervention, intervention_date: e.target.value })}
 ...
 {/* Add Button */}
                ...
                onClick={() => {
                if (
                newIntervention.staff_with_roles.length > 0 &&
                newIntervention.intervention_date
                ) {
      onChange([
        ...value,
        {
          staff_with_roles: newIntervention.staff_with_roles,
          intervention_date: newIntervention.intervention_date,
          observations: newIntervention.observations,
          //documentItem: newIntervention.file?.name || '',
        },
      ])
